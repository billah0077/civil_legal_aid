...
function getJob_postList(){
	$conn = OpenCon();
    if($_SESSION['user_type'] == 'user'){
        $sql = "select job_post.title,job_post.description,job_post.id,crime.title crime_name,court.name court_name from job_post LEFT JOIN crime ON job_post.crime_id = crime.id LEFT JOIN court ON job_post.court_id=court.id where job_post.user_id= ".$_SESSION['user_id'];        
    }
    elseif($_SESSION['user_type'] == 'admin' || 'lawyer'){
